...
const CONSENT_KEY = 'reg100_consent_accepted';
...
export default function ConsentModal() {
  ...
  useEffect(() => {
    let mounted = true;
    let recheckCount = 0;
    const maxRechecks = 5; // เพิ่มจาก 3 เป็น 5

    const performCheck = async () => {
      if (!mounted) return;
      
      // รอให้ form restoration ทำงานก่อน
      if (recheckCount === 0) {
        console.log('⏳ Waiting for form restoration to complete...');
      ...
      if (mounted && isLoading && recheckCount < maxRechecks) {
        recheckCount++;
        console.log(`🔄 Recheck attempt ${recheckCount}/${maxRechecks}`);
        setTimeout(performCheck, 1500); // เพิ่ม delay
      }
    };

    // Initial check with longer delay
    const initialTimeout = setTimeout(performCheck, 800); // เพิ่มจาก 300
  ...
  const checkConsentStatus = async () => {
    ...
    try {
      ...
      const draftData = getDraftFromLocal('register100');
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
        <div className="bg-white rounded-lg p-6 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <p className="text-gray-700">กำลังตรวจสอบ...</p>
          </div>
        </div>
      </div>
    );
