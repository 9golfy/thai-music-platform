    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          ...
          router.push(`/dashboard/register-support/${id}`);
        }, 2000);
      } else {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (data.success) {
        ...
        router.push('/dashboard/register-support');
      } else {
        alert('เกิดข้อผิดพลาด: ' + data.message);
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                {submission.supportType && <span className="font-medium">{submission.supportType}</span>}
            ...
            <div className="flex gap-3">
              {!isEditMode ? (
                <>
                  <button
                    onClick={() => router.push(`/dashboard/register-support/${id}?mode=edit`)}
        ...
        <StepSection title="Step 1: ข้อมูลพื้นฐาน" stepNumber={1}>
          <div className="grid grid-cols-2 gap-6">
            {submission.supportType && (
              <>
                <Field label="ประเภท" value={isEditMode ? editData?.supportType : submission.supportType} isEditMode={isEditMode} field="supportType" onChange={handleFieldChange} />
                <Field label="ชื่อประเภท" value={isEditMode ? editData?.supportTypeName : submission.supportTypeName} isEditMode={isEditMode} field="supportTypeName" onChange={handleFieldChange} />
                {submission.supportTypeMemberCount && (
                  <Field label="จำนวนสมาชิก" value={isEditMode ? editData?.supportTypeMemberCount : submission.supportTypeMemberCount} isEditMode={isEditMode} field="supportTypeMemberCount" onChange={handleFieldChange} />
                )}
              </>
            )}
