...
export default function RegisterSupportDataTable() {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await fetch('/api/register-support/list');
  ...
  const filteredSubmissions = submissions.filter(sub => {
    ...
    return (
      ...
      normalize(sub.supportType).includes(query)
    );
  });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/register-support/${deletingSubmission._id}`, {
        method: 'DELETE',
      });
      const data = await response.json();
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <input
                type="text"
                placeholder="ค้นหาโรงเรียน, จังหวัด, ผู้บริหาร, ประเภท..."
                value={searchTerm}
                ...
                filteredSubmissions.map((submission, index) => (
                  <tr key={submission._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      <div className="font-medium">{submission.supportType || '-'}</div>
                      {submission.supportTypeName && (
                        <div className="text-xs text-gray-400">{submission.supportTypeName}</div>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center text-sm">
                      <div className="flex items-center justify-center gap-2">
                        <a
                          href={`/dcp-admin/dashboard/register-support/${submission._id}`}
                        ...
                        <a
                          href={`/dcp-admin/dashboard/register-support/${submission._id}?mode=edit`}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                  <div>
                    ...
                    <p className="font-medium">{selectedSubmission.supportType || '-'}</p>
                  </div>
                  <div>
