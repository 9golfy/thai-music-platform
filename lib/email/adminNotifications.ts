...
export async function notifyAdminNewRegistration(
    ...
    <body>
      <div class="container">
        <div class="header">
          ...
          <p>Thai Music Platform - Admin Notification</p>
        </div>
  ...
  const text = `
Thai Music Platform - การลงทะเบียนใหม่
...
export async function notifyAdminPasswordReset(
  ...
  const text = `
Thai Music Platform - การขอรหัสผ่านใหม่
...
export async function sendDailySummaryToAdmin(
  ...
  const html = `
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>📊 สรุปรายวัน</h1>
          <p>Thai Music Platform - ${new Date().toLocaleDateString('th-TH')}</p>
        </div>
  ...
  const text = `
Thai Music Platform - สรุปรายวัน
...
ดูรายละเอียดในระบบ Admin: ${process.env.NEXT_PUBLIC_APP_URL}/dcp-admin/dashboard
  ...
  return sendEmail({
    to: ADMIN_EMAIL,
    subject: `📊 สรุปรายวัน Thai Music Platform - ${new Date().toLocaleDateString('th-TH')}`,
    html,
    text,
  });
}